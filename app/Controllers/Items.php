...
class Items extends Secure_Controller
{
    ...
    public function getView(int $item_id = NEW_ENTRY): void    // TODO: Long function. Perhaps we need to refactor out some methods.
    {
        ...
        $data['allow_temp_item'] = ($data['allow_temp_item'] === 1 && $item_id !== NEW_ENTRY && (int) $item_info->item_type !== ITEM_TEMP) ? 0 : 1;
        ...
        if ($item_id === NEW_ENTRY) {
            ...
            $item_info->taxable              = 1;
            $item_info->tax_exemption_reason = 'exempt';
        ...
        $data['standard_item_locked'] = (
            ...
            && (int) $item_info->item_type === ITEM_KIT
        ...
        if (! $use_destination_based_tax) {
            if ((int) $item_info->taxable === 0) {
                $data['tax_mode'] = 'none';
            } elseif (empty($data['item_tax_info'])) {
                $data['tax_mode'] = 'inherit';
            } else {
                $data['tax_mode'] = 'own';
            }

            $data['tax_exemption_reason'] = in_array($item_info->tax_exemption_reason, ['exempt', 'zero-rated'], true)
        ...
        if ($item_id !== NEW_ENTRY && (int) $item_info->item_id !== (int) $item_info->low_sell_item_id) {
    ...
    public function postSave(int $item_id = NEW_ENTRY): void
    {
        ...
        $use_destination_based_tax = (bool) $this->config['use_destination_based_tax'];
        $tax_mode                  = $this->request->getPost('tax_mode') ?? 'inherit';
        $tax_exemption_reason      = $this->request->getPost('tax_exemption_reason') ?? 'exempt';
        $tax_names                 = $this->request->getPost('tax_names') ?? [];
        $tax_percents              = $this->request->getPost('tax_percents') ?? [];
        $items_taxes_data          = [];

        if (! $use_destination_based_tax) {
            if (! in_array($tax_mode, ['inherit', 'own', 'none'], true)) {
                echo json_encode(['success' => false, 'message' => lang('Items.tax_mode_invalid'), 'id' => $item_id]);

                return;
            }

            if (! in_array($tax_exemption_reason, ['exempt', 'zero-rated'], true)) {
                $tax_exemption_reason = 'exempt';
            }

            if ($tax_mode === 'own') {
                if (! is_array($tax_names) || ! is_array($tax_percents)) {
                    echo json_encode(['success' => false, 'message' => lang('Items.tax_mode_invalid'), 'id' => $item_id]);

                    return;
                }

                foreach ($tax_percents as $tax_index => $tax_percent) {
                    if ($tax_percent === '') {
                        continue;
                    }

                    if (! is_string($tax_percent)) {
                        echo json_encode(['success' => false, 'message' => lang('Items.tax_percent_number'), 'id' => $item_id]);
                    ...
                    if (! is_numeric($tax_percentage) || $tax_percentage < 0 || $tax_percentage > 100) {
                        echo json_encode(['success' => false, 'message' => lang('Items.tax_percent_number'), 'id' => $item_id]);

                        return;
                    }

                    $tax_name = $tax_names[$tax_index] ?? lang('Items.sales_tax_1');
                ...
                if (empty($items_taxes_data)) {
                    echo json_encode(['success' => false, 'message' => lang('Items.tax_percent_required'), 'id' => $item_id]);
        ...
        $item_data = [
            ...
            'taxable'               => $use_destination_based_tax || $tax_mode !== 'none',
