 ...
 *
 * Used for the `dir` attribute on the page and for direction-aware styling.
 * Any locale whose base language is written right to left returns `rtl`.
 *
 * @param string|null $language_code Language code to inspect. Defaults to the current language.
 *
 * @return string Either `rtl` or `ltr`.
 */
function text_direction(?string $language_code = null): string
{
    $language_code ??= current_language_code();
    $base_language = strtolower(explode('-', str_replace('_', '-', $language_code))[0]);

    return in_array($base_language, RIGHT_TO_LEFT_LANGUAGES, true) ? 'rtl' : 'ltr';
}

/**
 * Returns true when the given language code is written right to left.
 *
 * @param string|null $language_code Language code to inspect. Defaults to the current language.
 */
function is_right_to_left(?string $language_code = null): bool
{
    return text_direction($language_code) === 'rtl';
}
