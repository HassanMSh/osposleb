...
class MailchimpConnector
{
    ...
    /**
     * Encrypted API key loaded from the application settings.
     */
    private string $_encrypted_api_key = '';    // TODO: Hungarian notation
    ...
    /**
     * Loads the Mailchimp settings without decrypting the stored API key.
     */
    public function __construct(string $api_key = '')
    {
        $config = config(OSPOS::class)->settings;

        // Upstream reads $this->config; the property does not exist, so this is always empty by design (defect D-004).
        ...
        if (! empty($mailchimp_api_key)) {
            $this->_encrypted_api_key = $mailchimp_api_key;
            $this->_api_key           = $api_key;
        }
    }

    /**
     * Resolves the configured API key only when a request needs it.
     */
    private function resolveApiKey(): void
    {
        if (empty($this->_api_key) && ! empty($this->_encrypted_api_key) && ! empty(config('Encryption')->key)) {
            $this->_api_key = Services::encrypter()->decrypt($this->_encrypted_api_key);
        }
     ...
     * Returns false when no API key can be resolved, including when the encryption key is missing.
     *
     ...
     */
    public function call(string $method, string $httpVerb = 'POST', array $args = []): array|bool
    {
        $this->resolveApiKey();
     ...
     * @param array  $args     Assoc array of parameters to be passed
     *
     * @return array|bool Assoc array of decoded result or False
     */
    private function _request(string $httpVerb, string $method, array $args = []): array|bool    // TODO: Hungarian notation
    {
        $result = false;
...
class Mailchimp_lib    // TODO: IMO We need to stick to the one class per file principle.
{
    ...
    /**
     * Creates a Mailchimp connector without decrypting configured credentials.
     */
     ...
     */
    public function getLists(array $parameters = ['fields' => 'lists.id,lists.name,lists.stats.member_count,lists.stats.merge_field_count']): array|bool
    {
     ...
     */
    public function getList(string $list_id, array $parameters = ['fields' => 'id,name,stats.member_count,stats.merge_field_count']): array|bool
    {
     ...
     */
    public function getMembers(string $list_id, int $count, int $offset, array $parameters = ['fields' => 'members.id,members.email_address,members.unique_email_id,members.status,members.merge_fields']): array|bool
    {
        $parameters += [
            'count'  => $count,
            'offset' => $offset,
     ...
     */
    public function getMemberInfoById(string $list_id, string $md5id, array $parameters = ['fields' => 'email_address,status,merge_fields']): array|bool
    {
     ...
     * @see http://developer.mailchimp.com/documentation/mailchimp/reference/lists/members/#read-get_lists_list_id_members_subscriber_hash
     */
    public function getMemberInfo(string $list_id, string $email, array $parameters = []): array|bool
    {
     ...
     */
    public function getMemberActivity(string $list_id, string $email, array $parameters = []): array|bool
    {
     ...
     */
    public function addMember(string $list_id, string $email, string $first_name, string $last_name, array $parameters = []): array|bool
    {
        $parameters += [
            'email_address' => $email,
            'status'        => 'subscribed',
     ...
     */
    public function removeMember(string $list_id, string $email): array|bool
    {
     ...
     */
    public function updateMember(string $list_id, string $email, string $first_name, string $last_name, array $parameters = []): array|bool
    {
        $parameters += [
            'status'       => 'subscribed',
            'merge_fields' => [
     ...
     */
    public function addOrUpdateMember(string $list_id, string $email, string $first_name, string $last_name, string $status, array $parameters = []): array|bool
    {
        $parameters += [
            'email_address' => $email,
            'status'        => $status,
