...
class Sale_lib
{
    ...
    {   // TODO: this needs to be converted to a switch statement
        ...
        if ($this->config['print_receipt_check_behaviour'] == 'never') {    // TODO: === ?
            return false;
        }   // Remember last setting, session based though
    ...
    {   // TODO: this needs to be converted to a switch statement
        ...
        if ($this->config['email_receipt_check_behaviour'] == 'never') {    // TODO: === ?
            return false;
        }   // Remember last setting, session based though
    ...
    public function is_customer_taxable(): bool    // TODO: This function is never called in the code
    {
        ...
        return $customer->taxable || $customer_id == -1;    // TODO: Replace with constant.  Also, I'm not sure we should be using the or operator instead of || here. $a || $b guarantees that the result of those two get returned.  It's possible that return $a or $b could return just the result of $a since `or` has a lower precedence.
    }
