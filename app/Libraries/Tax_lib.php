...
class Tax_lib
{
    ...
    /**
     * Computes taxes for all cart lines using live legacy-path TVA resolution.
     *
     * New sales use the item's no-TVA switch, then its own rate rows, then the
     * configured global rate. Existing sales use their stored tax rows so later
     * item or configuration changes cannot alter historical TVA.
     *
     * @param array $cart    Cart lines, updated with their tax markers.
     * @param int   $sale_id Existing sale id when resolving stored historical tax rows.
     *
     * @return array{0: array, 1: array} Sale-level and item-level tax rows.
     */
    public function get_taxes(array &$cart, int $sale_id = -1): array    // TODO: Replace -1 with constant.
    {
        ...
        $tax_decimals      = (int) ($this->config['tax_decimals'] ?? tax_decimals());
        $currency_decimals = (int) ($this->config['currency_decimals'] ?? totals_decimals());
        $tax_precision     = max(8, $currency_decimals + $tax_decimals + 4);
        ...
        if ($customer_id === -1 || $customer_info->taxable) {    // TODO: Replace -1 with constant.
            foreach ($cart as $line => $item) {
                ...
                if (! $this->config['use_destination_based_tax']) {
                    ...
                    if ($sale_id === -1) {    // TODO: Replace -1 with constant. Also, replace with ternary notation.
                        $item_info       = $this->item->get_info((int) $item['item_id']);
                        $item_is_taxable = ! property_exists($item_info, 'taxable') || (int) $item_info->taxable === 1;

                        if (! $item_is_taxable) {
                            $tax_reason = property_exists($item_info, 'tax_exemption_reason')
                                && in_array($item_info->tax_exemption_reason, ['exempt', 'zero-rated'], true)
                                ? $item_info->tax_exemption_reason
                                : 'exempt';
                            $tax_info = [[
                                'name'       => $tax_reason,
                                'percent'    => '0',
                            ...
                            if (empty($tax_info)) {
                                $global_rate = $this->config['default_tax_1_rate'] ?? '';

                                if ($global_rate !== '' && is_numeric($global_rate)) {
                                    $tax_info = [[
                                        'name' => ! empty($this->config['default_tax_1_name'])
                                            ? $this->config['default_tax_1_name']
                                            : lang('Items.sales_tax_1'),
                                        'percent' => (string) $global_rate,
                                    ]];
                                }
                            }
                        }
                    ...
                    foreach ($tax_info as $tax) {
                        ...
                        $tax_amount = '0.0';
                        $tax_reason = $tax['tax_reason'] ?? (in_array($tax['name'], ['exempt', 'zero-rated'], true) ? $tax['name'] : null);
                        ...
                        if ($this->config['tax_included']) {
                            ...
                            $tax_amount = bcmul(
                                $tax_basis,
                                bcdiv((string) $tax['percent'], '100', $tax_precision),
                                $tax_precision,
                            );
                        }

                        $tax_amount_is_nonzero = bccomp($tax_amount, '0', $tax_precision) !== 0;

                        if ($tax_amount_is_nonzero || $tax_reason !== null) {
                            $tax_group_sequence++;
                            $this->update_taxes($taxes, $tax_type, $tax['name'], $tax['percent'], $tax_basis, $tax_amount, $tax_group_sequence, Rounding_mode::HALF_UP, -1, $tax['name'], null, null, null, $tax_precision);    // TODO: Replace -1 with constant.
                            $tax_group_sequence++;    // TODO: $tax_group_sequence++;
                            $taxed = $taxed || $tax_amount_is_nonzero;
                        }
            ...
            $this->round_taxes(
                $taxes,
                $this->config['use_destination_based_tax'] ? null : $currency_decimals,
            );
        }
    ...
    /**
     * Computes a tax-inclusive amount without per-line rounding.
     *
     * @param int $tax_decimal   Retained for the public method signature.
     * @param int $rounding_code Retained for the public method signature.
     *
     * @return string Unrounded tax amount for the line.
     */
     ...
     *
     * @param int $scale Decimal precision used while accumulating rows.
     */
    public function update_taxes(array &$taxes, string $tax_type, string $tax_group, string $tax_rate, string $tax_basis, string $item_tax_amount, int $tax_group_sequence, int $rounding_code, int $sale_id, string $name = '', ?int $tax_code_id = null, ?int $jurisdiction_id = null, ?int $tax_category_id = null, int $scale = 4): void
    {
        ...
        if (! array_key_exists($tax_group_index, $taxes)) {
            ...
            $taxes[$tax_group_index]['sale_tax_basis']  = bcadd($taxes[$tax_group_index]['sale_tax_basis'], $tax_basis, $scale);
            $taxes[$tax_group_index]['sale_tax_amount'] = bcadd($taxes[$tax_group_index]['sale_tax_amount'], $item_tax_amount, $scale);
        }
    }
     ...
     *
     * @param int|null $decimals Override used by the legacy TVA path.
     */
    public function round_taxes(array &$taxes, ?int $decimals = null): void
    {
        if (! empty($taxes)) {
            $sort = [];
    ...
    public function apply_destination_tax(array &$item, string $city, string $state, int $sales_tax_code_id, string $register_mode, int $sale_id, array &$taxes, array &$item_taxes, int $line): bool
    {
        ...
        if ($tax_code_id !== -1 && (float) $item['price'] !== 0.0) {    // TODO: Replace -1 with constant.
            ...
            foreach ($tax_definition as $tax) {
                $cascade_sequence = (int) $tax['cascade_sequence'];
                ...
                if (bccomp((string) $tax_amount, '0', max(8, tax_decimals() + totals_decimals() + 4)) !== 0) {
                    $taxed = true;
