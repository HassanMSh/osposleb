            ...
            <div class="col-xs-8">
                <label class="radio-inline">
                    <?= form_radio([
                        ...
                        'id'      => 'stock_type',
                        'value'   => 0,
                        'checked' => (int) $item_info->stock_type === HAS_STOCK,
                    ]) ?> <?= lang('Items.stock') ?>
                </label>
                <label class="radio-inline">
                    <?= form_radio([
                        ...
                        'id'      => 'stock_type',
                        'value'   => 1,
                        'checked' => (int) $item_info->stock_type === HAS_NO_STOCK,
                    ]) ?><?= lang('Items.nonstock') ?>
                </label>
            </div>
        </div>
...
$radio_button = [
    ...
    'id'      => 'item_type',
    'value'   => 0,
    'checked' => (int) $item_info->item_type === ITEM,
];
...
$radio_button = [
    ...
    'id'      => 'item_type',
    'value'   => 1,
    'checked' => (int) $item_info->item_type === ITEM_KIT,
];
...
echo form_radio($radio_button) ?> <?= lang('Items.kit') ?>
                ...
                <?php if ($config['derive_sale_quantity'] === '1') { ?>
                    <label class="radio-inline">
                        <?= form_radio([
                            ...
                            'id'      => 'item_type',
                            'value'   => 2,
                            'checked' => (int) $item_info->item_type === ITEM_AMOUNT_ENTRY,
                ...
                <?php if ($allow_temp_item === 1) { ?>
                    <label class="radio-inline">
                        <?= form_radio([
                            ...
                            'id'      => 'item_type',
                            'value'   => 3,
                            'checked' => (int) $item_info->item_type === ITEM_TEMP,
                        ]) ?> <?= lang('Items.temp') ?>
                    </label>
                <?php } ?>
            </div>
        ...
        <?php if (! $use_destination_based_tax) { ?>
            <div class="form-group form-group-sm">
                <?= form_label(lang('Items.tax_mode'), 'tax_mode', ['class' => 'control-label col-xs-3']) ?>
                <div class="col-xs-8">
                    <label class="radio-inline">
                        <?= form_radio([
                            'name'    => 'tax_mode',
                    ...
                    <label class="radio-inline">
                        <?= form_radio([
                            ...
                            'id'      => 'tax_mode_own',
                            'value'   => 'own',
                            'checked' => $tax_mode === 'own',
                        ]) ?> <?= lang('Items.tax_mode_own') ?>
                    </label>
                    <label class="radio-inline">
                        <?= form_radio([
                            'name'    => 'tax_mode',
                            'id'      => 'tax_mode_none',
                            'value'   => 'none',
                            'checked' => $tax_mode === 'none',
                        ]) ?> <?= lang('Items.tax_mode_none') ?>
                    </label>
                    <div class="help-block"><?= esc(lang('Items.tax_mode_help')) ?></div>
                </div>
            </div>

            <div id="tax_own_rate_fields">
                <div class="form-group form-group-sm">
                    ...
                    <div class="col-xs-4">
                        <div class="input-group input-group-sm">
                            <?= form_input([
                                ...
                                'class' => 'form-control input-sm',
                                'value' => isset($item_tax_info[0]['percent']) ? to_tax_decimals($item_tax_info[0]['percent']) : '',
                            ]) ?>
                    ...
                    <div class="col-xs-4">
                        <div class="input-group input-group-sm">
                            <?= form_input([
                                ...
                                'value' => isset($item_tax_info[1]['percent']) ? to_tax_decimals($item_tax_info[1]['percent']) : '',
                            ]) ?>
                            <span class="input-group-addon input-sm"><b>%</b></span>
                        </div>
                    </div>
            ...
            <div id="tax_exemption_reason_fields" class="form-group form-group-sm">
                <?= form_label(lang('Items.tax_exemption_reason'), 'tax_exemption_reason', ['class' => 'control-label col-xs-3']) ?>
                <div class="col-xs-4">
                    <?= form_dropdown('tax_exemption_reason', [
                        'exempt'     => lang('Items.tax_reason_exempt'),
                        'zero-rated' => lang('Items.tax_reason_zero_rated'),
                    ], esc($tax_exemption_reason), ['class' => 'form-control input-sm']) ?>
                </div>
            </div>
        <?php } ?>
        ...
        <div class="form-group form-group-sm">
            ...
            <div class="col-xs-1">
                        <?= form_checkbox([
                            ...
                            'value'   => 1,
                            'checked' => (int) $item_info->allow_alt_description === 1,
                        ]) ?>
            </div>
        </div>
        ...
        <div class="form-group form-group-sm">
            ...
            <div class="col-xs-1">
                <?= form_checkbox([
                    ...
                    'id'      => 'is_serialized',
                    'value'   => 1,
                    'checked' => (int) $item_info->is_serialized === 1,
                ]) ?>
            </div>
        </div>
        ...
        <div class="form-group form-group-sm">
            ...
            <div class="col-xs-1">
                <?= form_checkbox([
                    ...
                    'id'      => 'is_deleted',
                    'value'   => 1,
                    'checked' => (int) $item_info->deleted === 1,
                ]) ?>
            </div>
        </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        var update_tax_mode_fields = function() {
            var tax_mode = $("input[name='tax_mode']:checked").val();
            $('#tax_own_rate_fields').toggle(tax_mode === 'own');
            $('#tax_exemption_reason_fields').toggle(tax_mode === 'none');
        };

        $("input[name='tax_mode']").change(update_tax_mode_fields);
        update_tax_mode_fields();
