...
archive_tmp=''
archive_path=''
container_defaults_path=''
container_defaults_installed=0

# Print the command-line help for the backup tool.
show_help() {
    cat <<'HELP'
Usage: scripts/backup.sh --destination <directory> [--db-container <name>] [--uploads <path>] [--env <path>]

Create one timestamped OSPOS backup archive.

Options:
  --destination <directory>  Existing, writable directory for the archive.
  --db-container <name>      Run mysqldump inside this Docker container.
...
# Copy the private defaults file into the requested database container.
install_container_defaults() {
    [[ -n $db_container ]] || return 0
    container_defaults_path="/tmp/ospos-backup-${BASHPID}.cnf"
    docker cp "$defaults_file" "$db_container:$container_defaults_path" >/dev/null \
        || fail "Could not copy the temporary MySQL defaults file into the Docker container."
    container_defaults_installed=1
}

# Run mysqldump using the private defaults file and the selected execution mode.
run_dump() {
    if [[ -n $db_container ]]; then
        docker exec "$db_container" mysqldump \
            --defaults-extra-file="$container_defaults_path" \
...
read_migration_version() {
    ...
    if [[ -n $db_container ]]; then
        result=$(docker exec "$db_container" mysql \
            --defaults-extra-file="$container_defaults_path" \
...
# Remove temporary files and any temporary defaults file in the Docker container.
cleanup() {
    if (( container_defaults_installed == 1 )); then
        docker exec "$db_container" rm -f "$container_defaults_path" >/dev/null 2>&1 || true
    fi
    ...
    while (( $# > 0 )); do
        ...
        case $option in
            ...
            --destination)
                ...
                ;;
            --db-container)
                (( $# >= 2 )) || fail "--db-container needs a container name."
                (( db_container_option_set == 0 )) || fail "--db-container was given more than once."
                db_container=$2
                db_container_option_set=1
                shift 2
                ;;
            --uploads)
...
if [[ -n $db_container ]]; then
    [[ $db_container =~ ^[A-Za-z0-9_][A-Za-z0-9_.-]*$ ]] || fail "Unsafe Docker container name."
...
read_env_value 'database.default.hostname'
db_host=$env_value
...
[[ -n $db_host ]] || fail 'database.default.hostname must not be empty.'
