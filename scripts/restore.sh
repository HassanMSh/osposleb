...
show_help() {
    cat <<'HELP'
Usage: scripts/restore.sh --archive <file> [--db-container <name>] [--uploads <path>] [--database <name>] [--yes]

Restore one OSPOS backup archive.

Options:
  --archive <file>           Backup archive to restore.
  --db-container <name>      Run mysql inside this Docker container.
...
install_container_defaults() {
    ...
    container_defaults_path="/tmp/ospos-restore-${BASHPID}.cnf"
...
# Run mysql with the private defaults file and the selected execution mode.
run_mysql() {
    if [[ -n $db_container ]]; then
        docker exec -i "$db_container" mysql \
            --defaults-extra-file="$container_defaults_path" "$target_database" "$@"
    else
